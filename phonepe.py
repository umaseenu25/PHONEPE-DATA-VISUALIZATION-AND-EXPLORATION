...
if select == "HOME":
    ...
    with col1:
        st.header("PHONEPE")
        st.subheader("ONE OF THE MOST POPULAR MONEY TRANSACTION APP IN INDIA")
        ...
        st.write("****FEATURES****")
        features = [
            "Recharge and pay bills = Mobile, FastTag, DTH, Water, Rent, etc.",
            "Check bank balance = History, Transaction Details, etc.",
            "Loans = Car, Bike, Gold, Home, Education, etc.",
            "Insurance = Health, Life, vechicle, etc.",
            "Wealth = SIP, Mutual Funds, etc."
        ]
        for feature in features:
            feature_name, feature_details = feature.split("=")
            st.write(f"**{feature_name.strip()}** = {feature_details.strip()}")

    with col2:
        images = ["im1.jpg", "im2.jpg"]
        for image in images:
            st.image(Image.open(image), width=300, caption=image)
